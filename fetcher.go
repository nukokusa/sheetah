...
type Fetcher interface {
	FetchSheet(ctx context.Context, spreadsheetID string, config *SheetConfig) (*Sheet, error)
...
func (f *fetcher) FetchSheet(ctx context.Context, spreadsheetID string, config *SheetConfig) (*Sheet, error) {
	req := f.ss.Spreadsheets.Get(spreadsheetID).IncludeGridData(true)
	req.Ranges(config.Name)
	...
	return NewSheet(spreadsheetID, config, resp)
}
